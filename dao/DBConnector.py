...
class DBConnector:
    ...
    #######################################################################
    # 데이터베이스파일 (jjstockdb.db)이 존재하는지 확인한다.
    # 존재하지 않을경우 db파일 generator를 실시한다.
    #######################################################################
    @staticmethod
    def exists_db_file():
        print('여기들어옵니까')
        if os.path.isfile("jjstockdb.db"):
            print('데이터베이스가 이미 존재합니다.')
        else:
            print('데이터베이스가 존재하지 않습니다. 데이터베이스 생성을 시작합니다.')
    ...
    #######################################################################
    ## 데이터베이스에 데이터를 쓰는 함수
    #  write() 함수는 테이블에 새로운 데이터를 입력하는 함수이다.
    #  @param table 데이터를 입력하고자 하는 대상이 되는 테이블
    #  @param columns 입력하고자 하는 대상이 되는 컬럼들(콤마로 구분되어있다).
    #  @param data 입력할 데이터들(콤마로 구분되어 있다)
    #######################################################################
